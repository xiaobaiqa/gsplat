...
CUDA_DEVICE=0
RESULT=results/benchmark_rc_15000_planGJ_garden/${SCENE}
LOG=results/benchmark_rc_15000_planGJ_garden.log

mkdir -p "$RESULT"
echo "[RUN] planGJ start $(date '+%F %T')" | tee -a "$LOG"
...
CUDA_VISIBLE_DEVICES=${CUDA_DEVICE} "$PY" simple_trainer.py residual_coverage \
  ...
  --strategy.coverage-min-late 0.008 \
  ...
  --strategy.prune-opacity-weight 0.30 \
  --strategy.prune-coverage-weight 0.20 \
  --strategy.prune-residual-weight 0.35 \
...
echo "[RUN] planGJ done $(date '+%F %T')" | tee -a "$LOG"
