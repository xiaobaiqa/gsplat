...
CUDA_DEVICE=0
RESULT=results/benchmark_rc_15000_planGH_garden/${SCENE}
LOG=results/benchmark_rc_15000_planGH_garden.log

mkdir -p "$RESULT"
echo "[RUN] planGH start $(date '+%F %T')" | tee -a "$LOG"
...
echo "[RUN] planGH done $(date '+%F %T')" | tee -a "$LOG"
