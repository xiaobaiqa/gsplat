...
RESULT=results/benchmark_rc_15000_planGP_garden/${SCENE}
LOG=results/benchmark_rc_15000_planGP_garden.log

mkdir -p "$RESULT"
echo "[RUN] planGP start $(date '+%F %T')" | tee -a "$LOG"
...
CUDA_VISIBLE_DEVICES=${CUDA_DEVICE} "$PY" simple_trainer.py residual_coverage \
  ...
  --strategy.coverage-min-late 0.01 \
  ...
  --strategy.prune-contribution-weight 0.22 \
...
echo "[RUN] planGP done $(date '+%F %T')" | tee -a "$LOG"
