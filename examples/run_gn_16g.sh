...
RESULT=results/benchmark_rc_15000_planGN_garden/${SCENE}
LOG=results/benchmark_rc_15000_planGN_garden.log

mkdir -p "$RESULT"
echo "[RUN] planGN start $(date '+%F %T')" | tee -a "$LOG"
...
CUDA_VISIBLE_DEVICES=${CUDA_DEVICE} "$PY" simple_trainer.py residual_coverage \
  ...
  --strategy.residual-threshold 0.12 \
  --strategy.coverage-min 0.005 \
  --strategy.target-coverage 0.20 \
  --strategy.residual-ema-decay 0.9 \
  --strategy.coverage-ema-decay 0.99 \
  --strategy.max-new-gs 96000 \
  --strategy.cap-max -1 \
  --strategy.residual-quantile 0.8 \
  --strategy.coverage-gate-min-score 0.01 \
  --strategy.coverage-score-power 0.75 \
  --strategy.prune-warmup-iter 8000 \
  --strategy.prune-opa-warmup-scale 0.15 \
  --strategy.replace-start-iter 4000 \
  --strategy.replace-budget-ratio 0.0 \
  ...
  --strategy.gate-score-late 0.02 \
  --strategy.coverage-min-early 0.02 \
  --strategy.coverage-min-late 0.005 \
  --strategy.residual-threshold-early-scale 0.92 \
  --strategy.residual-threshold-late-scale 1.10 \
  --strategy.contribution-ema-decay 0.96 \
  --strategy.prune-contribution-weight 0.35 \
  ...
  --strategy.growth-spike-ratio-limit 1.5 \
...
echo "[RUN] planGN done $(date '+%F %T')" | tee -a "$LOG"
