...
RESULT=results/benchmark_rc_15000_planGL_garden/${SCENE}
LOG=results/benchmark_rc_15000_planGL_garden.log

mkdir -p "$RESULT"
echo "[RUN] planGL start $(date '+%F %T')" | tee -a "$LOG"
...
CUDA_VISIBLE_DEVICES=${CUDA_DEVICE} "$PY" simple_trainer.py residual_coverage \
  ...
  --strategy.refine-stop-iter 12800 \
  --strategy.refine-every 110 \
  --strategy.reset-every 3200 \
  --strategy.no-absgrad \
  --strategy.no-revised-opacity \
  --strategy.key-for-gradient means2d \
  --strategy.growth-topk-ratio 0.38 \
  --strategy.residual-threshold 0.11 \
  --strategy.coverage-min 0.023 \
  --strategy.target-coverage 0.15 \
  --strategy.residual-ema-decay 0.9 \
  --strategy.coverage-ema-decay 0.99 \
  --strategy.max-new-gs 42000 \
  --strategy.cap-max 3850000 \
  ...
  --strategy.prune-opa-warmup-scale 0.24 \
  --strategy.replace-start-iter 6400 \
  --strategy.replace-budget-ratio 0.01 \
  ...
  --strategy.gate-score-late 0.033 \
  --strategy.coverage-min-early 0.02 \
  --strategy.coverage-min-late 0.010 \
  --strategy.residual-threshold-early-scale 0.93 \
  --strategy.residual-threshold-late-scale 1.04 \
  --strategy.contribution-ema-decay 0.965 \
  --strategy.prune-contribution-weight 0.34 \
  --strategy.prune-opacity-weight 0.28 \
  --strategy.prune-coverage-weight 0.18 \
  --strategy.prune-residual-weight 0.34 \
  --strategy.growth-spike-guard \
  --strategy.growth-spike-ratio-limit 1.30 \
...
echo "[RUN] planGL done $(date '+%F %T')" | tee -a "$LOG"
