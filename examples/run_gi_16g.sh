...
CUDA_DEVICE=0
RESULT=results/benchmark_rc_15000_planGI_garden/${SCENE}
LOG=results/benchmark_rc_15000_planGI_garden.log

mkdir -p "$RESULT"
echo "[RUN] planGI start $(date '+%F %T')" | tee -a "$LOG"
...
echo "[RUN] planGI done $(date '+%F %T')" | tee -a "$LOG"
