...
CUDA_DEVICE=0
RESULT=results/benchmark_rc_15000_planGD_garden/${SCENE}
LOG=results/benchmark_rc_15000_planGD_garden.log

mkdir -p "$RESULT"
echo "[RUN] planGD start $(date '+%F %T')" | tee -a "$LOG"
...
CUDA_VISIBLE_DEVICES=${CUDA_DEVICE} "$PY" simple_trainer.py residual_coverage \
  ...
  --strategy.max-new-gs 36000 \
  --strategy.cap-max 3450000 \
  --strategy.residual-quantile 0.8 \
  --strategy.coverage-gate-min-score 0.08 \
  --strategy.coverage-score-power 0.75 \
  --strategy.prune-warmup-iter 6200 \
  --strategy.prune-opa-warmup-scale 0.32 \
  --strategy.replace-start-iter 5600 \
  --strategy.replace-budget-ratio 0.015 \
  --strategy.min-replace-budget 64 \
  --strategy.max-replace-budget 2048 \
  --strategy.stage-transition-iter 4000 \
  --strategy.stage-end-iter 8500 \
  --strategy.gate-score-early 0.08 \
  --strategy.gate-score-late 0.055 \
  --strategy.coverage-min-early 0.02 \
  --strategy.coverage-min-late 0.02 \
  --strategy.residual-threshold-early-scale 0.90 \
  --strategy.residual-threshold-late-scale 1.13 \
  --strategy.contribution-ema-decay 0.96 \
  --strategy.prune-contribution-weight 0.40 \
  --strategy.prune-opacity-weight 0.42 \
  --strategy.prune-coverage-weight 0.28 \
...
echo "[RUN] planGD done $(date '+%F %T')" | tee -a "$LOG"
