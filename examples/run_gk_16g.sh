...
RESULT=results/benchmark_rc_15000_planGK_garden/${SCENE}
LOG=results/benchmark_rc_15000_planGK_garden.log

mkdir -p "$RESULT"
echo "[RUN] planGK start $(date '+%F %T')" | tee -a "$LOG"
...
CUDA_VISIBLE_DEVICES=${CUDA_DEVICE} "$PY" simple_trainer.py residual_coverage \
  ...
  --strategy.refine-every 105 \
  --strategy.reset-every 3100 \
  --strategy.no-absgrad \
  --strategy.no-revised-opacity \
  --strategy.key-for-gradient means2d \
  --strategy.growth-topk-ratio 0.42 \
  --strategy.residual-threshold 0.105 \
  ...
  --strategy.max-replace-budget 1280 \
  ...
  --strategy.residual-threshold-late-scale 1.03 \
  --strategy.contribution-ema-decay 0.965 \
  --strategy.prune-contribution-weight 0.30 \
  --strategy.prune-opacity-weight 0.33 \
  --strategy.prune-coverage-weight 0.20 \
  --strategy.prune-residual-weight 0.30 \
...
echo "[RUN] planGK done $(date '+%F %T')" | tee -a "$LOG"
