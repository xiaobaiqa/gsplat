...
CUDA_DEVICE=0
RESULT=results/benchmark_rc_15000_planGF_garden/${SCENE}
LOG=results/benchmark_rc_15000_planGF_garden.log

mkdir -p "$RESULT"
echo "[RUN] planGF start $(date '+%F %T')" | tee -a "$LOG"
...
CUDA_VISIBLE_DEVICES=${CUDA_DEVICE} "$PY" simple_trainer.py residual_coverage \
  ...
  --strategy.growth-topk-ratio 0.45 \
  ...
  --strategy.max-new-gs 60000 \
  --strategy.cap-max 3950000 \
  ...
  --strategy.max-replace-budget 1024 \
  ...
  --strategy.coverage-min-late 0.015 \
  ...
  --strategy.prune-contribution-weight 0.26 \
  --strategy.prune-opacity-weight 0.38 \
  --strategy.prune-coverage-weight 0.24 \
...
echo "[RUN] planGF done $(date '+%F %T')" | tee -a "$LOG"
